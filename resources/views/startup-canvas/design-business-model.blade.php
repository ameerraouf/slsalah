                            ...
                            <select class="form-select form-select-solid fw-bolder" id="contact"
                                ...
                                @foreach ($products as $product)
                                    <option value="{{ $product->id }}"
                                        @if (!empty($investor)) @if ($investor->product_id === $product->id)
                                            selected @endif
                                        @endif
                    ...
                    <table class="border-0 ">
                        ...
                        <tr>
                            <td style="padding:10px" colspan="2" rowspan="2">
                                <h6>{{ __('Problems') }}</h6>
                                <textarea class="bg-purple-light" name="problems" cols="35" rows="30" id="problems">
@if (!empty($model)){!! $model->problems !!}@else{{ old('problems') }}@endif
</textarea>
                            ...
                            <td style="padding:10px" colspan="2">
                                <h6>{{ __('Solutions') }}</h6>
                                <textarea name="solutions" cols="35" rows="15" id="solutions">
@if (!empty($model)){!! $model->solutions !!}@else{{ old('solutions') }}@endif
</textarea>
                            </td>
                            <td style="padding:10px" colspan="2" rowspan="2">
                                <h6>{{ __('Unique Value Proposition') }}</h6>

                                <textarea name="value_propositions" cols="25" rows="30" id="value">
@if (!empty($model)){!! $model->value_propositions !!}@else{{ old('value_propositions') }}@endif
</textarea>
                            </td>
                            <td style="padding:10px" colspan="2">
                                <h6>{{ __('Unfair Advantage') }}</h6>
                                <textarea name="unfair_advantage" cols="35" rows="15" id="advantage">
@if (!empty($model)){!! $model->unfair_advantage !!}@else{{ old('unfair_advantage') }}@endif
</textarea>
                            </td>
                            <td style="padding:10px" colspan="2" rowspan="2">
                                <h6>{{ __('Customer Segments') }}</h6>

                                <textarea name="customer_segments" cols="35" rows="30" id="customer_segments">
@if (!empty($model)){!! $model->customer_segments !!}@else{{ old('customer_segments') }}@endif
                        ...
                        <tr>
                            <td style="padding:10px" colspan="2">
                                ...
                                <textarea name="key_matrices" cols="35" rows="15"
                                    style="background-color:#C1F5D3; border-color:#C1F5D3; padding:10px" id="metrics">
@if (!empty($model)){!! $model->key_matrices !!}@else{{ old('key_matrices') }}@endif
</textarea>
                            </td>
                            <td style="padding:10px" colspan="2">
                                <h6>{{ __('Channels') }}</h6>

                                <textarea name="channels" cols="35" rows="15" id="channels">
@if (!empty($model)){!! $model->channels !!}@else{{ old('channels') }}@endif
                        ...
                        <tr>
                            <td style="padding:10px" colspan="5">
                                ...
                                <textarea name="cost_structure" cols="75" rows="15" id="cost_structure">
@if (!empty($model)){!! $model->cost_structure !!}@else{{ old('cost_structure') }}@endif
                            ...
                            <td style="padding:10px" colspan="5">
                                ...
                                <textarea name="revenue_stream" cols="75" rows="15" style=" padding:10px" id="revenue_stream">
