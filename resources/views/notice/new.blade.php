            ...
            <form action="/save-notice" method="post">
                ...
                <div class="col-lg-9 col-12 mx-auto">
                    ...
                    <div class="card card-body mt-4">
                        ...
                        <input type="text"
                            @if (!empty($notice)) value="{{ $notice->title }}" @else value="{{ old('title') }}" @endif
                            name="title" class="form-control" id="projectName">

                        <div class="mt-3">
                            <label class="">{{ __('Status') }}</label>

                            <select class="form-control" aria-label="Default select example" name="status">
                                <option value="Draft" @if (($notice->status ?? null) === 'Draft') selected @endif
                                    @if (old('status') === 'Draft') selected @endif>{{ __('Draft') }}
                                </option>
                                <option value="Published" @if (($notice->status ?? null) === 'Published') selected @endif
                                    @if (old('status') === 'Published') selected @endif>
                                    {{ __('Published') }}</option>
                        ...
                        <div class="form-group">
                            <textarea class="form-control" rows="10" id="description" name="notes">
@if (!empty($notice))
{{ $notice->notes }}
@else
{{ old('notes') }}
