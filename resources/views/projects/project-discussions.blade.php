                                    ...
                                    <div class=" me-3 ">
                                        ...
                                        @else
                                            <div
                                                class="avatar avatar-md rounded-circle bg-purple-light  border-radius-md p-2">
                                                <h6 class="text-purple mt-1">
                                                    {{ $users[$reply->admin_id]->first_name[0] }}{{ $users[$reply->admin_id]->last_name[0] }}
                                                </h6>
                                            </div>
                                        @endif
                                    </div>
                                    ...
                                    <h6 class="fw-bolder mt-0">
                                        @if ($reply->admin_id)
                                            @if (!empty($users[$reply->admin_id]))
                                                {{ $users[$reply->admin_id]->first_name }}
                                                {{ $users[$reply->admin_id]->last_name }}
                                            @endif
                                        @endif
                                    </h6>
                                    <p class="text-sm">
                        ...
                        <div class="d-flex mt-4">
                            ...
                            <div class="flex-grow-1 my-auto">
                                <form method="post" action="/save-project-message">
                                    ...
                                    <textarea class="form-control ms-2" name="message" placeholder="{{ __('Write your comment') }}" rows="2"></textarea>

                                    <input type="hidden" name="project_id" value="{{ $project->id }}">

                                    @csrf
                                    <button type="submit"
                                        class="btn btn-info  ms-2 btn-sm mt-3">{{ __('Send') }}</button>
                                </form>
                            </div>
                        </div>
                    </div>
