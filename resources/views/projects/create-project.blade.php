            ...
            <form action="/save-project" method="post">
                ...
                <div class="col-lg-9 col-12 mx-auto">
                    <div class=" card-body">
                        ...
                        <label for="projectName" class="form-label">{{ __('Product Name') }}</label><label
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-select" aria-label="Default select example" name="status">
                                <option value="Pending" @if (($project->status ?? null) === 'Pending') selected @endif
                                    @if (old('status') === 'Pending') selected @endif>{{ __('Pending') }}</option>
                                <option value="Started" @if (($project->status ?? null) === 'Started') selected @endif
                                    @if (old('status') === 'Started') selected @endif>{{ __('Started') }}</option>
                                <option value="Finished" @if (($project->status ?? null) === 'Finished') selected @endif
                                    @if (old('status') === 'Finished') selected @endif>{{ __('Finished') }}</option>
                            </select>
                        </div>
                        <div class="col-md-12">
                            <div>
                                <label for="exampleFormControlInput1"
                                    class="form-label">{{ __('Team Members') }}</label><span class="text-danger">*</span>
                                <select class="form-control select2" multiple id="" name="members[]">
                                    @foreach ($other_users as $other_user)
                                        <option value="{{ $other_user->id }}"
                                            @if ($members) @if (in_array($other_user->id, $members)) selected @endif
                                            @endif
                        ...
                        <div class="row mt-4">
                            ...
                            <div class="col-6">
                                ...
                                <input class="form-control" name="end_date" id="end_date"
                                    @if (!empty($project)) value="{{ $project->end_date }}"
                                       @else
                                       value="{{ date('Y-m-d') }}" @endif>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            <textarea class="form-control" rows="10" id="description" name="description">{{ $project->description ?? (old('description') ?? '') }}</textarea>
                        </div>
                        @csrf
                        @if ($project)
