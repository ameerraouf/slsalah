...
@section('content')
    <div class="card bg-purple-light mb-3 mt-4">
        <div class="card-header bg-purple-light pb-0 p-3">
            <div class="row">
                <div class="col-md-8 ">
                    <h6 >{{__('Working capital planning')}} </h6>
                    <p>رأس المال العامل هو مقدار رأس المال الذى يستخدم بجميع عمليات المشروع ويعد مؤشر قوى لتقييم الاستثمار بالمشروع</p>
                </div>
                <div class="col-md-4 text-right">

                    <a class="btn bg-gradient-dark" href="{{ route('workingInvested.create') }}"><i class="fas fa-plus"></i>&nbsp;&nbsp;
                        {{__('Add')}}
                    </a>
                    <a href="{{ route('workingInvested.show') }}" class="btn bg-dark-alt text-white">
        ...
        <div class="col-12">
            <div class="card card-body mb-4">
                <div class="card-body px-0 pt-0 pb-2">
                    <div class="row">
                        <p>{{__('Elements of working capital')}}</p>
                    </div>
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0" id="cloudonex_table">
                            ...
                            <tr>
                                ...
                                <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-2">{{__('monthly cost')}}</th>
                                <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-2">{{__('annual cost')}}</th>
                                <th class="text-secondary opacity-7"></th>
                            </tr>
                            <tbody>
                            @if(count($workingInvested) > 0)
                                @foreach($workingInvested as $investor)
                                        ...
                                        <td class="align-middle text-right">
                                            <div class="ms-auto">
                                                <a class="btn btn-link text-dark px-3 mb-0"
                                                   href="{{ route('workingInvested.create', ['id' =>$investor->id]) }}"><i
                                                            class="fas fa-pencil-alt text-dark me-2"
                                                            aria-hidden="true"></i>{{__('Edit')}}</a>

                                                <button type="button" class="btn btn-link text-danger text-gradient px-3 mb-0" onclick="deleteItem('{{ route('workingInvested.destroyCost', ['id' =>$investor->id]) }}')"
                                                   ><i
                                                            class="far fa-trash-alt me-2"></i>{{__('DeleteCost')}}</button>
                                                <button type="button" class="btn btn-link text-danger text-gradient px-3 mb-0" onclick="deleteItem('{{ route('workingInvested.destroy', ['id' =>$investor->id]) }}')"
                                                   ><i
