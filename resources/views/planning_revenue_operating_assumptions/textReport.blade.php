            ...
            <div class="card card-body mb-4">
                <div class="card-body px-0 pt-0 pb-2">
                    ...
                    <div class="table-responsive p-0">
                        ...
                        <p>وبالتالي فإن إجمالي التكاليف في السنة الأولى سيكون ( {{  formatCurrency($calc_total['total_cost_first_year_as_number'] + $calc_total['first_year_profit_before_zakat_percent_number'],getWorkspaceCurrency($settings) ) }}) وفي السنة الثانية سيكون ( {{  formatCurrency($calc_total['total_cost_second_year_as_number'] + $calc_total['second_year_profit_before_zakat_percent_number'],getWorkspaceCurrency($settings) ) }}) وفي السنة الثالثة سيكون ( {{  formatCurrency($calc_total['total_cost_third_year_as_number'] + $calc_total['third_year_profit_before_zakat_percent_number'],getWorkspaceCurrency($settings) ) }})</p>
                        ...
                        <p>***بالنسبة لقائمة التدفقات النقدية من الأنشطة التشغيلية فصافي الربح في السنة الأولى ( {{ $calc_total['net_profit_first_year_as_string'] }}) وفي السنة الثانية ( {{ $calc_total['net_profit_second_year_as_string']}}) وفي السنة الثالثة ( {{ $calc_total['net_profit_third_year_as_string'] }}) .</p>
                        <p>***وبالنسبة للتغير في رأس المال العامل فقيمتة في السنة الأولى ( {{ $calc_total['net_profit_first_year_as_number'] < 0 ? "SAR 0" :$calc_total['first_year_capital_change'] }}) و في السنة الثانية ({{ $calc_total['second_year_capital_change']}}) وفي السنة الثالثة ({{ $calc_total['third_year_capital_change']}})</p>
                        <p>***وبالنسبة لصافي التدفق النقدي من الأنشطة التشغيلية  في السنة الأولى ف ({{$first_year_net_cash_flow}}) وفي السنة الثانية ({{$second_year_net_cash_flow}}) وفي السنة الثالثة ({{$third_year_net_cash_flow}})</p>
                        <p> ***و نموذج الاستثمار الرأسمالي ف إجمالي الإيرادات في السنة الأولى فقيمتها ( {{$calc_total['first_year_after_operating_assumption_as_string'] }}) وفي السنة الثانية فقيمتها ( {{ $calc_total['second_year_after_operating_assumption_as_string']}}) وفي السنة الثالثة فقيمتها ( {{ $calc_total['third_year_after_operating_assumption_as_string'] }})</p>
                        <p>***و إجمالي التكاليف قيمته في السنة الأولى ( {{formatCurrency($calc_total['total_cost_first_year_as_number'] + $calc_total['first_year_profit_before_zakat_percent_number'],getWorkspaceCurrency($settings) )  }}) وفي السنة الثانية ( {{ formatCurrency($calc_total['total_cost_second_year_as_number'] + $calc_total['second_year_profit_before_zakat_percent_number'],getWorkspaceCurrency($settings) )  }}) وفي السنة الثالثة فقيمته ( {{ formatCurrency($calc_total['total_cost_third_year_as_number'] + $calc_total['third_year_profit_before_zakat_percent_number'],getWorkspaceCurrency($settings) ) }})</p>
                        <p>***و صافي الربح قيمته في السنة الأولى ( {{ $calc_total['net_profit_first_year_as_string'] }}) وفي السنة الثانية ({{ $calc_total['net_profit_second_year_as_string'] }}) وفي السنة الثالثة فقيمته ({{ $calc_total['net_profit_third_year_as_string']}})</p>
                        <p>***اما بالنسبة لرأس مال المستثمر في فترة التأسيس ف قيمته (SAR {{ $totalInvestedCapital }})</p>
                        <p>***وبذلك يكون التدفق النقدي السنوي في السنة الأولى ( {{ $first_year_net_cash_flow }}) وفي السنة الثانية ( {{$second_year_net_cash_flow }}) وفي السنة الثالثة ( {{ $third_year_net_cash_flow }})</p>
                    </div>
                </div>
            </div>
        </div>
