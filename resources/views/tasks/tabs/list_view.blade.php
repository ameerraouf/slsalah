    ...
    <table class="table align-items-center mb-0" id="cloudonex_table">
        ...
        <thead>
            ...
            <tr>
                ...
                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                    {{ __('Start Date') }}</th>
                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">{{ __('Due Date') }}
                </th>
                    ...
                    <td class=" text-sm fw-bolder mb-0">
                        ...
                        <div class="d-flex">
                                ...
                                @if (isset($users[$task->contact_id]))
                                    @if (!empty($users[$task->contact_id]->photo))
                                        <a href="javascript:" class="avatar avatar-sm rounded-circle"
                                            ...
                                            title="{{ $users[$task->contact_id]->first_name }}">
                                            <img
                                                src="{{ PUBLIC_DIR }}/uploads/{{ $users[$task->contact_id]->photo }}">
                                        </a>
                                    @else
                                        <div class="avatar  avatar-sm rounded-circle bg-indigo">
                                            <p class=" mt-3 text-white text-uppercase">
                                                {{ $users[$task->contact_id]->first_name[0] }}{{ $users[$task->contact_id]->last_name[0] }}
                                            </p>
                                        </div>
                                    @endif
                                @endif
                            ...
                            <div class="text-sm fw-bold mt-2 ms-1 ">
                                @if (isset($users[$task->contact_id]))
                                    {{ $users[$task->contact_id]->first_name }}
                                    {{ $users[$task->contact_id]->last_name }}
                                @endif
                            </div>
                        </div>
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                ...
                                @if ($task->status !== 'in_progress')
                                    <li><a class="dropdown-item change_task_status" data-id="{{ $task->id }}"
                                            ...
                                            href="#">{{ __('Mark as In Progress') }}</a></li>
                                @endif
                ...
                <form method="post" id="form_main">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="">
                            ...
                            <input type="text" id="input_name" name="subject"
                                @if (!empty($task)) value="{{ $task->subject }}" @else value="{{ old('subject') }}" @endif
                        ...
                        <div class="mb-1 mt-2">
                            ...
                            <label for="exampleFormControlInput1" class="form-label">{{ __('Assigned To') }}</label>
                            <select class="form-select form-select-solid fw-bolder" id="contact"
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}"
                                        @if (!empty($task)) @if ($task->contact_id === $user->id)
                                            selected @endif
                                        @endif
...
{{ old('description') }} @endif
    ...
    <div class="modal fade" tabindex="-1" id="kt_modal_2">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ __('strategic_objective') }}</h5>

                </div>

                <form method="post" id="form_goal_main">
                    <input type="hidden" name="task_goal_id" value="{{ isset($tasksGoal) ? $tasksGoal->id : '' }}">
                    <div class="modal-body">
