    ...
    <div class="row">
        ...
        <div class="col text-end">
            ...
            <a href="/gantt" type="button" class="btn btn-secondary text-white">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-clock ms-2"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
                {{__(' Gantt Chart ')}}
            </a>
            <button type="button" class="btn bg-dark-alt text-white" id="btn_add_new_category">
                <i class="fa fa-plus"></i>{{__(' Add Task ')}}
            </button>
            <button type="button" class="btn bg-dark-alt text-white" id="btn_add_goal">
                <i class="fa fa-plus ms-2"></i>{{ __('Add'). ' ' . __('strategic_objective')}}
            </button>
            <a href="{{ route('admin.tasks.show') }}" class="btn bg-dark-alt text-white">
                <i class="fa fa-eye"></i>
            </a>
        </div>
    </div>
...
@section('script')
    <script>
        $(function () {
            ...
            $('#form_goal_main').on('submit', function (event) {
                event.preventDefault();
                $btn_submit.prop('disabled', true);
                $.post('{{route('admin.tasksGoals.save')}}', $('#form_goal_main').serialize()).done(function () {
                    location.reload();
                }).fail(function (data) {
                    let obj = $.parseJSON(data.responseText);
                    $btn_submit.prop('disabled', false);
            ...
            let myModal2 = new bootstrap.Modal(document.getElementById('kt_modal_2'), {
                keyboard: false
            });
