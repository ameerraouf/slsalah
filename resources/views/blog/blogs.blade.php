    ...
    @if (!count($blogs))
        ...
        <div class="card">
            <div class="card-body">
                ...
                <a href="/write-blog" type="button" class="btn btn-info text-white"><i class="fas fa-plus"></i>
                    {{ __('Write an article ') }}</a>
            </div>
        </div>
    @else
        <div class="row">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mt-3">
                                <a href="/view-blog?id={{ $blog->id }}"
                                    class="btn btn-success btn-xs mb-0">{{ __('Read') }}</a>
                                <a href="/write-blog?id={{ $blog->id }}"
                                    class="btn btn-info btn-xs mb-0">{{ __('Edit') }}</a>
                                <a href="/delete/blog/{{ $blog->id }}" class="btn btn-warning btn-xs mb-0 delete-btn">
                                    {{ __('Delete') }}</a>
                            </div>
